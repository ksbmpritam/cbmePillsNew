...
if ($_POST['data'] != '' && $_POST['user_id'] != '' && $_POST['exam_id'] != '' && $_POST['Percentage'] != '') {
    ...
    foreach ($obj as $data) {
        $query = mysqli_query($con, 'INSERT INTO `result`(`question`, `option_1`, `option_2`, `option_3`, `option_4`, `correct_answer`,  `user_answer`, `q_id`, `exam_id`, `user_id`, `date`,`track`,`Percentage`,`image`) VALUES '
                ...
                . '"' . strip_tags($data->{'option_1'}) . '",'
                . '"' . strip_tags($data->{'option_2'}) . '",'
                . '"' . strip_tags($data->{'option_3'}) . '",'
                . '"' . strip_tags($data->{'option_4'}) . '",'
                . '"' . strip_tags($data->{'correct_answer'}) . '",'
...
//        $total = $right - ($wrong * 0.25);
        ...
        echo json_encode(array("status" => "1", "Response" => $output, "time" => $examrow['exam_time'], "total" => count($output), "right" => $right, "image" => $img, "percentage" => $reper . '%', 'result' => $text, "wrong_ans" => $wrong, "skipped" => $skip, "attempted" => $attempted, 'total_marks' => $total,'from_marks' => $attemptedpoints));
    } else {
