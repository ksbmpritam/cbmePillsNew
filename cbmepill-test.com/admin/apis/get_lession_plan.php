...
if($method == 'GET' && $_GET['id']) {
    if($_GET['id']){
        $result = getLessonPlanById($con,$_GET['id']);
    ...
    if($_GET['category_id']){
        $result = getLessonPlansWithDetails($con,$_GET['category_id']);
        echo json_encode($result);
    }else{
        echo json_encode(array("success" => "false", 'message' => 'Category Id Required'));   
    }
}else{
...
function getLessonPlansWithDetails($con,$category_id)
{
    $res = [];
    $sql = "SELECT * FROM lesson_plan WHERE status = 1 AND category_id= $category_id ";
