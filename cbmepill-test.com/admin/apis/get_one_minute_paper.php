...
if($method == 'GET' && $_GET['id']) {
    if($_GET['id']){
        $result = getOneMinutePaperById($con,$_GET['id']);
        ...
        $result = getOneMinutePaperDetails($con);
        echo json_encode($result);
}else{
...
function getOneMinutePaperDetails($con)
{
    $res = [];
    $sql = "SELECT * FROM one_minute_paper WHERE status = 1 ORDER BY id DESC";
